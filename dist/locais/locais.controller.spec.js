...
describe('LocaisController', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            ...
            providers: [
                {
                    provide: locais_service_1.LocaisService,
                    useValue: mockLocaisService,
                },
            ],
        }).compile();
        controller = module.get(locais_controller_1.LocaisController);
    ...
    describe('POST /api/locais - criar', () => {
        ...
        it('deve tratar erros ao criar local', async () => {
            ...
            await expect(controller.criar(criarLocalDto)).rejects.toThrow(common_1.HttpException);
        });
    });
    describe('GET /api/locais - listar', () => {
        ...
        it('deve filtrar locais por tipo', async () => {
            ...
            mockLocaisService.listar.mockResolvedValue(mockResultado);
            const query = { tipo: local_schema_1.TipoLocal.CASA_PRAIA };
            const result = await controller.listar(query);
            expect(result.success).toBe(true);
            expect(result.data.locais).toHaveLength(1);
            expect(result.data.locais[0].tipo).toBe(local_schema_1.TipoLocal.CASA_PRAIA);
        });
        it('deve filtrar locais por faixa de preço', async () => {
            ...
            const result = await controller.listar(query);
            expect(result.success).toBe(true);
            expect(mockLocaisService.listar).toHaveBeenCalledWith(expect.objectContaining({ precoMin: 400, precoMax: 600 }));
        });
        it('deve buscar locais por proximidade', async () => {
            ...
            expect(mockLocaisService.listar).toHaveBeenCalledWith(expect.objectContaining({ lat: -23.5178, lon: -46.1894, raioKm: 30 }));
        });
        ...
        it('deve tratar erros ao listar locais', async () => {
            ...
            const query = {};
            await expect(controller.listar(query)).rejects.toThrow(common_1.HttpException);
        });
    });
    describe('GET /api/locais/:id - buscarPorId', () => {
        ...
        it('deve lançar erro quando local não encontrado', async () => {
            const notFoundError = new common_1.HttpException('Local não encontrado', common_1.HttpStatus.NOT_FOUND);
            mockLocaisService.buscarPorId.mockRejectedValue(notFoundError);
            await expect(controller.buscarPorId('507f1f77bcf86cd799439099')).rejects.toThrow(common_1.HttpException);
        });
        it('deve validar ID inválido', async () => {
            await expect(controller.buscarPorId('id-invalido')).rejects.toThrow(common_1.HttpException);
    ...
    describe('PATCH /api/locais/:id - atualizar', () => {
        ...
        it('deve lançar erro quando local não encontrado para atualizar', async () => {
            ...
            await expect(controller.atualizar('507f1f77bcf86cd799439099', atualizarLocalDto)).rejects.toThrow(common_1.HttpException);
        });
        it('deve validar ID inválido', async () => {
            const atualizarLocalDto = {
                preco: 600,
            };
            await expect(controller.atualizar('id-invalido', atualizarLocalDto)).rejects.toThrow(common_1.HttpException);
            expect(mockLocaisService.atualizar).not.toHaveBeenCalled();
        });
    });
});
//# sourceMappingURL=locais.controller.spec.js.map