...
describe('CidadesService', () => {
  ...
  describe('buscarCidadePorCoordenadas', () => {
    it('deve retornar cidade quando encontrada no MongoDB', async () => {
      const mockCidade = {
        ...
        localizacao: {
          type: 'Point',
          coordinates: [-46.1894, -23.5178], // [longitude, latitude]
        },
      };

      // Mock do query builder do Mongoose - limit retorna Promise diretamente
      const mockQuery = {
  ...
  describe('obterCidadeAtual', () => {
    ...
    it('deve buscar na API quando não encontrar no MongoDB', async () => {
      ...
      jest.spyOn(service, 'salvarCidade').mockResolvedValue(null as any);
